// Explicit local host fixture. Creates and detaches only its own 16 MiB image.
...
try{
  ...
  execFileSync('/usr/bin/hdiutil',['create','-size','16m','-fs','HFS+','-volname','PiSpokeDisposable','-type','UDIF','-nospotlight',join(root,'fixture.dmg')],{timeout:15000});
  const plist=execFileSync('/usr/bin/hdiutil',['attach','-nobrowse','-mountpoint',mount,'-plist',join(root,'fixture.dmg')],{timeout:15000});
  const parsed=JSON.parse(spawnSync('/usr/bin/plutil',['-convert','json','-o','-','-'],{input:plist,encoding:'utf8'}).stdout);
  device=parsed['system-entities'].find(entity=>/^\/dev\/disk\d+$/.test(entity['dev-entry']))?.['dev-entry'];
  if(!device||!parsed['system-entities'].some(entity=>entity['mount-point']===mount))throw Error('Disposable mount identity could not be confirmed');
  ...
  if(device){try{execFileSync('/usr/bin/hdiutil',['detach',device],{timeout:15000});detached=true;}catch(error){throw Error(`Cleanup unconfirmed: inspect ${device} mounted under ${root}; fixture retained`,{cause:error});}}
  await rm(root,{recursive:true,force:true});
  if(closeError)throw closeError;
}
const result={status:'PASS',scope:'actual disposable 16 MiB HFS+ volume; mounted-topology rejection and real ENOSPC artifact failure',...evidence,cleanup:detached?'detached and removed':'not mounted'};
