...
export async function httpProvider(reply: (request: FakeRequest, index: number) => { text?: string; tool?: { name: string; arguments: object }; delay?: number }) {
  ...
  const server = createServer(async (req, res) => {
    ...
    chunk({}, response.tool ? 'tool_calls' : 'stop'); res.end('data: [DONE]\n\n');
  });
