...
test('S32 characterization: detached descendant stays contained; wrapper exit is never reported as verified cleanup', async () => {
  ...
  const descendant = `setTimeout(()=>{const fs=require('node:fs');let denied=false;try{fs.writeFileSync(${JSON.stringify(source)},'bad')}catch(e){denied=['EPERM','EACCES'].includes(e.code)}fs.writeFileSync(${JSON.stringify(marker)},String(denied));},200);`;
  const parent = `require('node:child_process').spawn(process.execPath,['-e',${JSON.stringify(descendant)}],{detached:true,stdio:'ignore'}).unref()`;
  try {
    ...
    await vi.waitFor(async () => expect(await readFile(marker, 'utf8')).toBe('true'), { timeout: 5000 });
